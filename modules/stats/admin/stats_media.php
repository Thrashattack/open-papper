 ...
 * @version     $Id: stats_media.php,v 1.1 2008/01/07 17:53:16 roufneck Exp $
 */
...
/**
 * @admintitle  View media stats
 * @adminnav    3
 * @return  string  admin screen html content
 */
function admin_stats_media()
{
    ...
    if (empty($GLOBALS['PROPS_ERRORSTACK'])) {
        ...
        $output .=
            ...
            .'  <tr>'.LF
            .'    <th>' . props_gettext("Caption") . '</th>'.LF
        ...
        $q  = "SELECT *, SUM(hits) as sum_hits FROM props_stats_log "
            . "LEFT JOIN props_media ON props_stats_log.id = props_media.media_id "
            . "WHERE props_stats_log.command = 'media' "
        ...
        if (!sql_num_rows($result)) {
            ...
            while($row = sql_fetch_object($result)) {
                ...
                .'    <td>' . htmlspecialchars($row->caption) . '</td>'.LF
                .'    <td>' . htmlspecialchars($row->type) . '</td>'.LF
