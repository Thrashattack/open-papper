 ...
 * @version     $Id: poll_edit_comments.php,v 1.7 2007/12/11 15:46:31 roufneck Exp $
 */

/**
 * @admintitle  Edit poll comments
 * @adminnav    0
 * @return  string  admin screen html content
 */
function admin_poll_edit_comments()
{
    // Set sitebar
    ...
    switch($op) {
        ...
        case "delete_comment":
            ...
            if (empty($GLOBALS['PROPS_ERRORSTACK'])) {
                $q  = "UPDATE props_polls_comments SET "
                    . "deleted = 1, "
                    . "timestamp = timestamp "
    ...
    $q  = "SELECT t1.*, t2.username, t2.user_type FROM props_polls_comments AS t1 "
    ...
    if (!sql_num_rows($result)) {
         ...
         while ($row = sql_fetch_object($result)) {
            if ($row->user_id == 0) {
                ...
                $username =
                     '<span class="'.$class.'">'
                    .'<a href="./?module=users&amp;function=edit_user&amp;user_id=' . $row->user_id . '">' . htmlspecialchars($row->username) . '</a>'
                    .'</span>';
            }
            $output .=
                ...
                .'    <td style="text-align: center;"><a href="./?module=' . $GLOBALS['PROPS_MODULE'] . '&amp;function=' . $GLOBALS['PROPS_FUNCTION'] . '&amp;op=delete_comment&amp;poll_id=' . $poll_id . '&amp;comment_id=' . $row->comment_id . '&amp;pageID=' . props_pageID() . '">[' . props_gettext("Delete") . ']</td>'.LF
                .'  </tr>'.LF;
