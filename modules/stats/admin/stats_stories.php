 ...
 * @version     $Id: stats_stories.php,v 1.2 2008/01/07 17:53:16 roufneck Exp $
 */
...
/**
 * @admintitle  View story stats
 * @adminnav    2
 * @return  string  admin screen html content
 */
function admin_stats_stories()
{
    ...
    if (empty($GLOBALS['PROPS_ERRORSTACK'])) {
        ...
        $output .=
            ...
            .'  <tr>'.LF
            .'    <th>' . props_gettext("Headline") . '</th>'.LF
            .'    <th>' . props_gettext("Section") . '</th>'.LF
            .'    <th>' . props_gettext("Published") . '</th>'.LF
            .'    <th>' . props_gettext("Modified") . '</th>'.LF
        ...
        if (!sql_num_rows($result)) {
            ...
            while($row = sql_fetch_object($result)) {
                ...
                .'    <td>' . htmlspecialchars($row->headline) . '</td>'.LF
                .'    <td>' . htmlspecialchars(section_shortname($row->section_id)) . '</td>'.LF
