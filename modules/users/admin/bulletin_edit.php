 ...
 * @version     $Id: bulletin_edit.php,v 1.15 2007/12/11 15:46:32 roufneck Exp $
 */

/**
 * @admintitle  Edit bulletin
 * @adminprivs  bulletin_delete  Delete bulletin
 * @adminprivs  wysiswg_editor  Use WYSIWYG editor
 * @adminnav    0
 * @return  string  admin screen html content
 */
function admin_bulletin_edit()
{
    // Set sitebar
    ...
    $bulletin_id = props_getrequest('bulletin_id', VALIDATE_INT);
    ...
    switch($op) {
        ...
        case props_gettext("Delete"):
            if (!admin_has_priv($GLOBALS['PROPS_MODULE'], 'bulletin_delete')) {
            ...
            sql_query("DELETE FROM props_bulletins_subscriptions WHERE bulletin_id = $bulletin_id");
            sql_query("DELETE FROM props_bulletins WHERE bulletin_id = $bulletin_id");
        ...
        case props_gettext("Save"):
            ...
            if (sql_num_rows(sql_query("SELECT * FROM props_bulletins WHERE bulletin_name = '" . sql_escape_string($bulletin_name) . "' AND bulletin_id != $bulletin_id"))) {
                $GLOBALS['PROPS_ERRORSTACK']['bulletin_name']['message'] = props_gettext("This name already exists.");
            }

            if (sql_num_rows(sql_query("SELECT * FROM props_bulletins WHERE bulletin_shortname = '" . sql_escape_string($bulletin_shortname) . "' AND bulletin_id != $bulletin_id"))) {
        ...
        default:
            ...
            $bulletin_shortname = $bulletin->bulletin_shortname;
            $from_name = $bulletin->from_name;
            $from_email = $bulletin->from_email;
            $bounce_email = $bulletin->bounce_email;
