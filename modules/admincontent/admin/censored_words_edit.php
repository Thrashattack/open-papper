 ...
 * @version     $Id: censored_words_edit.php,v 1.4 2008/01/09 17:50:03 roufneck Exp $
 */

/**
 * @admintitle  Edit censored words
 * @adminnav    0
 * @return  string  admin screen html content
 */
function admin_censored_words_edit()
{
    // Set sitebar
    ...
    $q  = "SELECT * FROM props_censored_words "
        . "WHERE censored_id = $censored_id";
    $result = sql_query($q);
    $censored_word = sql_fetch_object($result);
    ...
    switch($op) {
        ...
        case props_gettext("Save"):
            ...
            if (sql_num_rows(sql_query("SELECT censored_id FROM props_censored_words WHERE pattern = '".sql_escape_string($pattern)."' AND censored_id != $censored_id"))) {
            ...
            if (empty($GLOBALS['PROPS_ERRORSTACK'])) {
                // Update record
                $q  = "UPDATE props_censored_words SET "
    ...
    $output =
        ...
        .'<input name="censored_id" type="hidden" value="' . $censored_id . '" />'.LF
        .'<fieldset>'.LF
